...
const CartItemRow: React.FC<CartItemRowProps> = ({ item, onUpdateQuantity, onRemoveItem }) => {
  ...
  return (
    <div className="glass-panel p-5 rounded-2xl border border-slate-800 flex items-center gap-4 group">
      <div className="w-20 h-20 rounded-xl bg-slate-900 p-2 flex items-center justify-center shrink-0">
        <img
          ...
          <div className="glass-panel p-6 rounded-3xl border border-slate-800 space-y-6 h-fit sticky top-24">
            ...
            <div className="space-y-3 text-xs">
              <div className="flex justify-between text-slate-400">
                <span>Tổng số lượng mục</span>
                <span className="font-semibold text-white">{cart?.totalItems || items.length}</span>
              </div>
              <div className="flex justify-between text-slate-400">
                <span>Tạm tính</span>
            ...
            <button
              ...
              className="w-full py-3.5 rounded-xl bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-500 hover:to-violet-500 text-white font-bold text-sm shadow-xl shadow-indigo-500/30 flex items-center justify-center gap-2 transition-all hover:scale-105"
            >
